...
const PlaylistItem = ({ video, active, played }) => (
  <StyledPlaylistItem active={active} played={played}>
    ...
    <div className='player__video-time'>
      {video.duration}
    </div>
  </StyledPlaylistItem>
)
...
const StyledPlaylistItem = styled.div`
  ...
  background: none;
  border: ${(props) =>
    props.active ? '2px dotted #696969' : 'none'};
  font-size: 1.6em;
  ...
  ::before {
    display: ${(props) =>
      props.active || !props.played ? 'none' : 'block'};
    content: '';
    width: 10px;
    min-height: 10px;
    background: #409f4e;
  ...
  .player__video-download {
    flex-grow: 1; /* default 0 */
    color: ${(props) =>
      props.played ? '#fff' : props.theme.color};
    margin: 0 0 0 20px;
    max-width: 30px;
    padding: 0 20px;
